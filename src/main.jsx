...
import { Heart, Search, ShoppingCart, X, Trash2, Plus, AlertTriangle, Loader, CheckCircle } from 'lucide-react';
...
// 
// This config is used when running locally (outside the Canvas environment) 
// and the global variable __firebase_config is MISSING.
// ----------------------------------------------------------------------
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const initializeFirebase = async () => {
      ...
      try {
        currentAppId = typeof __app_id !== 'undefined'
          ? __app_id
          : 'default-app-id';
        ...
        if (!firebaseConfig) {
          ...
          isLocalRun = true;
          console.warn("FIREBASE INIT: Using LOCAL_FIREBASE_CONFIG fallback for local testing.");
        }

        if (!firebaseConfig || !firebaseConfig.apiKey || firebaseConfig.apiKey.includes('YOUR_API_KEY')) {
          throw new Error("Missing or invalid Firebase Configuration. Please provide your actual config object.");
        }


        console.log(`FIREBASE INIT: Config loaded (Local Fallback: ${isLocalRun}).`);
        ...
        if (!isLocalRun && initialAuthToken) { // Only use token if we are in the hosted environment
        ...
        const unsubscribeAuth = onAuthStateChanged(newAuth, (user) => {
          if (user) {
            ...
            console.log("FIREBASE AUTH: User is unauthenticated (signed out/anonymous).");
          }
          setIsAuthReady(true);
        });
        ...
        console.error("Firebase Initialization Failed (CRITICAL ERROR):", error);
  ...
  useEffect(() => {
    ...
    console.log(`FIRESTORE: Listening to path: /artifacts/${appId}/public/data/artifacts`);
    ...
    const unsubscribe = onSnapshot(artifactsQuery, (snapshot) => {
      ...
      console.log(`FIRESTORE: Successfully fetched ${fetchedArtifacts.length} artifacts.`);
    }, (error) => {
      console.error("Firestore Snapshot Error (Check Security Rules):", error);
      // This usually means a Security Rule violation, or the database is not set up.
      setDbError(`Failed to fetch artifacts (Security or Setup Issue): ${error.message}.`);
    });
  ...
  const handleAddArtifact = async (newArtifact) => {
    const collectionRef = getArtifactsCollectionRef;
    if (!collectionRef) {
      console.error('Database connection error. Cannot add artifact. (Collection Ref is null)');
      return false;
    }
    ...
    try {
      ...
      setDbError(`Add Error: ${error.message}. Please check your Firestore Security Rules to ensure authenticated users can write to the public collection.`);
  ...
  const handleDeleteArtifact = async (id) => {
    ...
    // IMPORTANT: Replacing confirm() with a custom modal check (which is not implemented yet)
    ...
    try {
      ...
      console.log(`Artifact ${id} deleted.`);
    } catch (error) {
  ...
  const handleAddToCart = useCallback((artifact) => {
    setCartItems(prev => {
      const existingItem = prev.find(item => item.id === artifact.id);
      if (existingItem) {
        return prev.map(item =>
          item.id === artifact.id ? { ...item, quantity: item.quantity + 1 } : item
        );
      }
        ...
        <div className="max-w-7xl mx-auto p-4 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="text-gray-600 font-semibold text-sm">
              User: {isAuthReady ? displayUserId(userId) : <span className="text-yellow-600">Authenticating...</span>}
            </div>

            <button onClick={() => setShowCartModal(true)} className="relative p-2 text-indigo-600 hover:text-indigo-800 transition duration-150">
      ...
      <div className="w-full h-48 bg-gray-100 flex items-center justify-center overflow-hidden">
        {artifact.imageUrl ? (
          <img
            ...
            onError={(e) => { e.target.onerror = null; e.target.src = 'https://placehold.co/400x300/e9d5ff/4c1d95?text=Image+Missing'; }}
          />
        ) : (
          <div className="text-gray-400 text-sm p-4 text-center">{artifact.name || 'No Image URL Provided'}</div>
        )}
      </div>

      <div className="p-5 flex flex-col flex-grow">
        <h3 className="text-xl font-bold text-indigo-900 mb-2">{artifact.name}</h3>
        <p className="text-sm text-gray-600 flex-grow mb-3 line-clamp-2">{artifact.description}</p>
        <p className="text-2xl font-extrabold text-indigo-600 mb-4">
          KES {artifact.price.toLocaleString('en-KE')}
        </p>
        <div className="flex justify-between items-center mt-auto">
          <button
            onClick={() => handleAddToCart(artifact)}
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-8 transform transition-all duration-300 scale-100">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-3xl font-bold text-indigo-900">Add New Artifact (Admin)</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">Artifact Name</label>
              <input
                type="text"
                name="name"
                id="name"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="price" className="block text-sm font-medium text-gray-700 mb-1">Price (KES)</label>
                <input
                  type="number"
                  name="price"
                  id="price"
              ...
              <div>
                <label htmlFor="imageUrl" className="block text-sm font-medium text-gray-700 mb-1">Product Image URL</label>
                <input
                  type="url"
                  name="imageUrl"
                  id="imageUrl"
  ...
  if (!isAuthReady || (!db && !dbError)) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-8">
        ...
        <p className="text-xl font-medium text-indigo-800 mb-2">Connecting to Makindu Artifacts Database...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
        ...
        {dbError && (
          <div className="mt-8 bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-xl relative shadow-md">
            ...
            <p className="text-xs mt-3 italic">
              **Action:** Please check the browser console for specific errors. If you are using a local fallback config, ensure your **Firebase Security Rules** permit read/write access for authenticated or anonymous users.
            </p>
          </div>
        )}

        <div className="mt-16">
          <h2 className="text-4xl font-extrabold text-indigo-900 mb-8">Featured Artifacts</h2>
          ...
          {artifacts.length > 0 ? (
            ...
            <div className="bg-white p-12 rounded-xl text-center shadow-lg border border-gray-200">
              <p className="text-2xl font-medium text-gray-700">No artifacts found in the database yet.</p>
              <p className="mt-4 text-gray-500">
                Please use the **+ Admin** button to add the first product to the 
                <code className="bg-gray-100 p-1 rounded text-sm text-indigo-600">
                  /artifacts/{appId}/public/data/artifacts
                </code> collection in Firestore.
              </p>
            </div>
          )}
        </div>
      ...
      {/* Placeholder Cart Modal (Future Phase 2) */}
      {showCartModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-8">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-3xl font-bold text-indigo-900">Your Cart ({totalCartItems})</h2>
              <button onClick={() => setShowCartModal(false)} className="p-2 rounded-full hover:bg-gray-100 text-gray-500">
                <X className="h-6 w-6" />
              </button>
            </div>
            {cartItems.length > 0 ? (
