...
import {
  ...
  serverTimestamp,
  deleteDoc,
  addDoc, // <-- ADDED for simpler artifact saving
...
export const saveArtifact = async (artifactData) => {
  ...
  try {
    ...
    const docRef = await addDoc(collection(db, artifactsCollectionPath), {
      ...artifactData,
      timestamp: serverTimestamp(),
      // The ID is the document ID generated by addDoc, which we can retrieve later if needed
    });
    console.log("Artifact saved with ID:", docRef.id);
    return docRef.id;
  } catch (error) {
...
// --- Exports ---
export { db, auth, userId, appId, firebaseConfig, isAuthReady };
